...
def injScanPlot(inj, data, err, dataName, saveto, chip, fit=False):
	...
	plt.errorbar(inj,amp1_p,yerr=err1,label='Amp1',marker="o",linestyle='None',color='r')
...
injection=[i*0.1 for i in range(3,16)] #1,19
homeDir = "/Users/asteinhe/AstroPixData/astropixOut_tmp"
saveDir = "/Users/asteinhe/AstroPixData/astropixOut_tmp/injectionScans/v2/"
#runList = [[102221,1],[102221,2]] #day of run, pixel number #v1
runList=[['030122',1]]
binsize=0.01 if traceInteg else 0.001
...
for i,element in enumerate(runList):
	...
	for inj in injection:
		...
		popt, enRes, pcov = enResFitting.enResPlot(settings, savedir=saveDir, integral=traceInteg, injection=True, binSize=binsize)
